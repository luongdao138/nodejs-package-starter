...
export abstract class TransactionBaseService {
  ...
  withTransaction(transactionManager?: EntityManager): this {
    ...
    const cloned = new (this.constructor as any)(this.__container__, this.__configModule__, this.__moduleDeclaration__)
  ...
  protected shouldRetryTransaction_(err: { code: string } | Record<string, unknown>): boolean {
  ...
  protected async atomicPhase_<TResult, TError>(
    ...
    isolationOrErrorHandler?: IsolationLevel | ((error: TError) => Promise<never | TResult | void>),
    ...
    let isolation: IsolationLevel | ((error: TError) => Promise<never | TResult | void>) | undefined | null =
    ...
    if (this.transactionManager_) {
      ...
      if (isolation && this.manager_) {
        let result
        try {
          result = await this.manager_.transaction(isolation as IsolationLevel, async (m) => doWork(m))
          return result
        } catch (error) {
